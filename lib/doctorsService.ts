...
export async function fetchDoctorsBySpecialization(specialization: string): Promise<Doctor[]> {
  try {
    if (__DEV__) console.log(`🔍 [fetchDoctorsBySpecialization] Searching providers_doctor_details for: "${specialization}"`);
    ...
    if (__DEV__) console.log(`✅ [fetchDoctorsBySpecialization] Found ${doctors.length} doctors`);
    return doctors;
  } catch (error) {
