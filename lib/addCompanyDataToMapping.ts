...
export const addCompanyDataToMapping = async () => {
  try {
    if (__DEV__) console.log('🔧 Creating a simple company verification record...');
    ...
    if (companyError) {
      if (__DEV__) console.log('⚠️ Company creation failed (RLS policy):', companyError.message);
      if (__DEV__) console.log('🔧 Trying alternative approach - using existing company ID...');
      
      // Just log that we're using the existing company ID
      if (__DEV__) console.log(`✅ Using existing company ID: ${existingCompanyId}`);
    } else {
      if (__DEV__) console.log('✅ Created company record:', companyData);
    }
    
    // All mappings already use the same company ID, so no need to update
    if (__DEV__) console.log('🎉 All mappings are already using the correct company ID!');
