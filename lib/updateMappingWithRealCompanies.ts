...
export const updateMappingWithRealCompanies = async () => {
  try {
    if (__DEV__) console.log('🔧 Updating mapping with real company IDs...');
    ...
    if (!companies || companies.length === 0) {
      if (__DEV__) console.log('❌ No companies found in providers_company_verification table');
      return;
    }
    
    if (__DEV__) console.log(`📋 Found ${companies.length} companies:`, companies.map(c => ({ id: c.id, name: c.company_name })));
    ...
    if (__DEV__) console.log(`📋 Found ${mappings?.length || 0} mapping records`);
    ...
    if (mappings && mappings.length > 0) {
      for (let i = 0; i < mappings.length; i++) {
        ...
        if (__DEV__) console.log(`🔄 Updating ${mapping.service_name} with company ${companies[companyIndex].company_name}`);
        ...
        if (updateError) {
          ...
          if (__DEV__) console.log(`✅ Updated ${mapping.service_name}:`, updateResult);
        }
      }
    }
    
    if (__DEV__) console.log('🎉 All mappings updated with real company IDs!');
