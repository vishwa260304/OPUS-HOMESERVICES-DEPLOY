...
export default function SupportOrdersScreen() {
  ...
  const answers: { [key: string]: string } = {
    'Can I change the delivery address of my order?': 'Once an order is placed, it cannot be routed to another address. You can return your order and place a new order after updating the delivery address.',
    'Is there a minimum order value?': 'We do not have a minimum or maximum order value. Order as much or as little as you need. A variable small cart fee may be added based on your order value.',
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
        <View style={[styles.sectionHeader, { backgroundColor: colors.surface }]}><Text style={[styles.sectionHeaderText, { color: colors.text }]}>Order / Products Related</Text></View>
        {questions.map((q, i) => (
          <View key={i}>
