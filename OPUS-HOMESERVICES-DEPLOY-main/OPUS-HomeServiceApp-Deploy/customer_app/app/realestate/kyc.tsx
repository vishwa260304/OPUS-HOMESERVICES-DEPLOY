...
import { createKycVerification, verifyKycDocumentWithOCR } from '../../lib/kyc';
      ...
      >
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerCenter}>
            ...
            <Text style={[styles.headerSubtitle, { color: colors.textSecondary }]}>Step 2 of 3</Text>
          </View>
        ...
        <View style={[styles.bottomContainer, { backgroundColor: colors.background, borderTopColor: colors.border }]}>
          <TouchableOpacity
            ...
            onPress={async () => {
              ...
              if (!aadhaar || !pan) {
                Alert.alert('KYC', 'Please enter both Aadhaar and PAN numbers.');
                return;
              }
              if (!aadhaarImage || !panImage) {
                Alert.alert('KYC', 'Please upload Aadhaar and PAN photos.');
                return;
              }
              try {
                ...
                await createKycVerification({
                  ...
                  panNumber: pan,
                  aadhaarLocalUri: aadhaarImage,
                  panLocalUri: panImage,
                });
                hapticSuccess();
                const { name, address, phone, houseType, houseAddress } = params as any;
                router.push({ pathname: '/realestate/kyc-success', params: { name, address, phone, houseType, houseAddress } });
              } catch (err: any) {
                console.error('KYC submit failed', err);
                Alert.alert('KYC', err?.message || 'Failed to submit KYC');
              }
            }}
          >
            <LinearGradient
