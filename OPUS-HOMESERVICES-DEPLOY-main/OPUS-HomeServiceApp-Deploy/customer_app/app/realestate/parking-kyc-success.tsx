...
export default function ParkingKycSuccessScreen() {
      ...
      <View style={styles.content}>
        ...
        >
          ...
          <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
            Your parking space has been added to our listings and is now live.
          </Text>
          ...
          <View style={styles.infoCards}>
            <View style={[styles.infoCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <View style={styles.infoCardText}>
                <Text style={[styles.infoCardTitle, { color: colors.text }]}>Parking Listed</Text>
          ...
          <View style={[styles.propertyCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={styles.propertyHeader}>
              <Ionicons name="car-sport-outline" size={24} color={colors.secondary} />
            ...
            <View style={styles.propertyDetails}>
              <View style={styles.propertyDetailItem}>
                ...
                <Text style={[styles.propertyDetailText, { color: colors.textSecondary }]} numberOfLines={1}>
                  {`${(params as any).parkingLocation || 'Location'}${(params as any).buildingName ? ', ' + (params as any).buildingName : ''}`}
                </Text>
              </View>
              <View style={styles.propertyDetailItem}>
                <Ionicons name="pricetag-outline" size={18} color={colors.textSecondary} />
                <Text style={[styles.propertyDetailText, { color: colors.textSecondary }]}>
                  {`${(params as any).rentAmount || '—'} ${((params as any).rentPeriod) || ''}`}
                </Text>
              </View>
              <View style={styles.propertyDetailItem}>
      ...
      <View style={[styles.bottomContainer, { backgroundColor: colors.background, borderTopColor: colors.border }]}>
        <TouchableOpacity
          ...
          onPress={async () => {
            hapticButtonPress();
            try {
              const existingRaw = await AsyncStorage.getItem('user_properties');
              ...
              const primaryImage = photos[0] ? { uri: photos[0] } : require('../../assets/images/Aframe.webp');
              const listing = {
                id,
                title: 'Car Parking for Rent',
                location: `${(params as any).parkingLocation || ''}${(params as any).buildingName ? ', ' + (params as any).buildingName : ''}`,
                price: `${(params as any).rentAmount || '—'} ${((params as any).rentPeriod) || ''}`.trim(),
                deposit: (params as any).securityDeposit ? `+ Deposit ${(params as any).securityDeposit}` : '',
                area: `${(params as any).length || '—'} x ${(params as any).width || '—'} ${(params as any).length || (params as any).width ? '(ft/m)' : ''}`.trim(),
                areaType: `Parking • ${(params as any).parkingType || ''} • ${(params as any).vehicleAllowed || ''}`.trim(),
                postedDays: 0,
                dealerName: 'You',
                isVerified: true,
                isFeatured: true,
                imageCount: Math.max(1, photos.length),
                category: 'Residential',
                subcategory: 'Parking for Rent',
                image: primaryImage,
              };
              const updated = [listing, ...existing];
              await AsyncStorage.setItem('user_properties', JSON.stringify(updated));
            } catch {}
