  ...
  getProvidersByServiceName: async (serviceName: string, serviceType?: string): Promise<{ data: ServiceProvider[] | null; error: any }> => {
    try {
      ...
      if (!normalizedServiceName) {
        console.log(`[getProvidersByServiceName] No service name provided.`);
        return { data: [], error: null };
      }

      console.log(`[getProvidersByServiceName] Fetching ALL providers for: "${normalizedServiceName}" with type: "${serviceType}"`);
      ...
      if (!allMappings || allMappings.length === 0) {
        console.log(`[getProvidersByServiceName] No providers found for service type: "${serviceType}"`);
        return { data: [], error: null };
      }
      ...
      console.log(`[getProvidersByServiceName] Found ${filteredMappings.length} provider(s) out of ${allMappings.length} total for "${normalizedServiceName}"`);
      ...
      console.log('[RATING] Step 1: Provider service IDs to fetch ratings for:', providerServiceIds);
      ...
      try {
        ...
        console.log('[RATING] Step 2: Fetching user_ids from providers_services...');
        const { data: servicesData, error: servicesError } = await supabase
          .from('providers_services')
          .select('id, user_id')
          .in('id', providerServiceIds);

        console.log('[RATING] Step 3: Services data:', servicesData);

        if (servicesError) {
          console.log('[RATING] ❌ RLS Error - Cannot access providers_services table');
          console.log('[RATING] Please add this RLS policy in Supabase:');
          console.log('[RATING] CREATE POLICY "Allow public read" ON providers_services FOR SELECT TO public USING (true);');
        }
        ...
        if (!servicesError && servicesData && servicesData.length > 0) {
          ...
          for (const service of servicesData) {
            ...
            console.log(`[RATING] Step 4: Fetching all services for provider ${service.user_id}...`);
            ...
            if (!allServicesError && allUserServices && allUserServices.length > 0) {
              ...
              console.log(`[RATING] Step 5: Provider has ${allServiceIds.length} services: [${allServiceIds.join(', ')}]`);
              ...
              console.log(`[RATING] Step 6: Found ${reviewsData?.length || 0} reviews across all services`);
              ...
              if (!reviewsError && reviewsData && reviewsData.length > 0) {
                ...
                if (ratings.length > 0) {
                  ...
                  console.log(`[RATING] ✅ Service ${service.id}: ${avg.toFixed(1)} stars (${ratings.length} reviews across services: ${allServiceIds.join(', ')})`);
                }
              }
            }
          }
          ...
          console.log('[RATING] Step 7: Fetching avatars...');
          ...
          if (userIds.length > 0) {
            ...
            if (!profilesError && profilesData) {
              ...
              console.log('[RATING] Step 8: Avatars fetched successfully');
            }
          }
        }
      } catch (error) {
