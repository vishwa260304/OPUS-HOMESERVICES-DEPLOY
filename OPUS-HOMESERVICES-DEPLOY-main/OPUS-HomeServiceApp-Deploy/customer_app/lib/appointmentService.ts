...
export async function createDoctorAppointmentBooking(
  ...
  try {
    ...
    console.log('✅ Appointment booking created successfully:', booking.id);
    ...
    if (doctor.userId) {
      try {
        ...
        console.log('✅ Patient details saved to patients table');
...
export function subscribeToBookingStatus(
  ...
  const channel = supabase
    ...
    .subscribe((status) => {
      console.log('📡 Booking status subscription:', status);
    });
