...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const signInWithPhone = async (phone: string): Promise<{ success: boolean; message: string }> => {
    try {
      ...
      console.log(`[Auth] Attempting signInWithOtp for: ${phone}`);
  ...
  const verifyOTP = async (phone: string, otp: string): Promise<{ success: boolean; message: string }> => {
    try {
      setLoading(true);
      console.log(`[Auth] Attempting verifyOtp for: ${phone}`);
      ...
      if (data.session) {
        ...
        try {
          console.log('[Auth] Loading profile for user:', data.session.user.id);
        ...
        try {
          console.log('[Auth] Persisting auth metadata to SecureStore');
