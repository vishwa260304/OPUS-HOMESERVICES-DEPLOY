...
export default function CartScreen() {
  ...
  const fetchServiceProviders = async (serviceId: string, serviceName: string, serviceType?: string, category?: string) => {
    ...
    try {
      if (__DEV__) console.log(`[Cart] Fetching providers for service: "${serviceName}", type: "${serviceType}", category: "${category}"`);
      ...
      if (error) {
        ...
        if (__DEV__) console.log(`[Cart] Filtered to ${filteredProviders.length} provider(s) for "${serviceName}" (category: "${category}")`);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView
        ...
        contentContainerStyle={{ paddingTop: insets.top, paddingBottom: 100 + insets.bottom, backgroundColor: colors.background }}
        refreshControl={
          <RefreshControl
